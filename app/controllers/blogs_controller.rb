class BlogsController < ApplicationController
  before_action :set_side_bar_topics, except: [:destroy, :create, :update, :toggle_status]
  ...
  def index
    if logged_in?(:site_admin)
      @blogs = Blog.sort_by_recent.page(params[:page]).per(5)
    else
      @blogs = Blog.sort_by_recent.published.page(params[:page]).per(5)
    end
    set_title("My Blog")
  end
